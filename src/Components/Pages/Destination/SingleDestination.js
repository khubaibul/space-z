...
const SingleDestination = ({ destination, handleId }) => {
    ...
    return (
        <div className='lg:flex justify-around'>
            ...
            <div className='lg:w-1/3 md:w-full mt-10 md:flex md:flex-col sm:w-full sm:flex sm:flex-col items-center'>
                <div className='hidden md:block lg:hidden sm:block'>
                    <DestinationNavbar handleId={handleId} destination={destination} />
                </div>
                <div>
                    <h1 className='text-white text-8xl uppercase md:text-center lg:text-start sm:text-center md:mt-8 sm:mt-8' style={{ fontFamily: "'Bellefair', sans-serif" }}>{name}</h1>
                    <p className='text-secondary lg:w-full md:w-[85%] sm:w-[85%] my-4 text-xl leading-10 lg:text-start md:text-center md:mx-auto sm:text-center sm:mx-auto' style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>{description}</p>
                </div>
                <div className="lg:w-full w-[85%] h-[1px] opacity-25 bg-white"></div>
                <div className='lg:w-full flex gap-x-20 mt-7 sm:flex-col lg:flex-row md:flex-row sm:gap-y-6'>
                    <div>
